...
// ===== SECRET ADMIN MENU & VOICE MODE =====

// Admin menu state
let adminTapCount = 0;
let adminTapTimer = null;
const ADMIN_TAP_THRESHOLD = 15;
const ADMIN_TAP_TIMEOUT = 2000; // 2 seconds for "rapid succession"
const ADMIN_PASSWORD = 'admin';

// Voice Mode state
let voiceModeActive = false;
let voiceModeState = 'idle'; // 'idle', 'listening', 'processing'
let voiceModeData = {
    length: null,
    t9String: null,
    bIdentity: null,
...
// Secret admin menu: 15 rapid taps in center
let centerTapArea = null;
function initSecretAdminMenu() {
    // Create invisible center tap area
    centerTapArea = document.createElement('div');
    centerTapArea.style.position = 'fixed';
    centerTapArea.style.top = '40%';
    centerTapArea.style.left = '30%';
    centerTapArea.style.width = '40%';
    centerTapArea.style.height = '20%';
    centerTapArea.style.zIndex = '9999';
    centerTapArea.style.background = 'transparent';
    document.body.appendChild(centerTapArea);
    
    centerTapArea.addEventListener('click', handleAdminTap);
    centerTapArea.addEventListener('touchend', handleAdminTap);
}
...
function showVoiceMode() {
    const voiceScreen = document.getElementById('voiceModeScreen');
    if (voiceScreen) {
        ...
        updateVoiceStatus('Tap bottom half to start microphone');
        updateVoiceResults('');
        
        // Make bottom half clickable
        voiceScreen.addEventListener('click', handleVoiceModeTap);
        voiceScreen.addEventListener('touchend', handleVoiceModeTap);
    }
}
...
function handleVoiceModeTap(e) {
    ...
    // Check if tap is in bottom half
    if (clickY > screenHeight / 2) {
        if (voiceModeState === 'idle' || voiceModeState === 'processing') {
            startListening();
        }
    }
}
...
function initVoiceRecognition() {
    if ('webkitSpeechRecognition' in window || 'SpeechRecognition' in window) {
        ...
        console.log('Web Speech API not supported, will use OpenAI Whisper API');
    }
}
...
function startListening() {
    ...
    if (recognition) {
        try {
            ...
            console.log('Web Speech API not available, using OpenAI Whisper');
...
function stopListening() {
    ...
    // Also stop OpenAI recording if active
    stopOpenAIRecording();
...
function handleVoiceResult(event) {
    ...
    if (finalTranscript) {
        ...
        processVoiceInput(accumulatedTranscript.trim());
...
function handleVoiceError(event) {
    ...
    if (event.error === 'no-speech') {
        ...
        setTimeout(() => {
            if (voiceModeActive && voiceModeState === 'listening') {
                startListening();
            }
        }, 1000);
...
// Process voice input and check for trigger words
function processVoiceInput(text) {
    const lowerText = text.toLowerCase();
    
    // Check for trigger words
    if (lowerText.includes('digits')) {
        handleTriggerDigits();
        return;
    }
    
    if (lowerText.includes('blank')) {
        handleTriggerBlank(text);
        return;
    }
    
    if (lowerText.includes('okay') || lowerText.includes('ok')) {
        handleTriggerOkay(text);
        return;
    }
    
    if (lowerText.includes('interesting')) {
        handleTriggerInteresting(text);
        return;
    }
    
    // Process based on current state
    if (voiceModeState === 'listening') {
        if (voiceModeData.length === null) {
            // Listening for length
            extractLength(text);
        } else if (voiceModeData.t9String === null) {
            // Listening for T9 string
            extractT9String(text);
        } else if (voiceModeData.bIdentity === null) {
            // Listening for B-IDENTITY
            extractBIdentity(text);
        }
    }
}

function handleTriggerDigits() {
    voiceModeData.length = null;
    voiceModeState = 'listening';
    updateVoiceStatus('Listening for length (number of digits)...');
    updateVoiceResults('');
}

function handleTriggerBlank(text) {
    // Extract length if not already set
    if (voiceModeData.length === null) {
        extractLength(text);
    }
    
    // Apply length filter
    if (voiceModeData.length !== null && currentFilteredWords.length > 0) {
        applyLengthFilter();
    }
    
    // Start listening for T9 string
    voiceModeData.t9String = null;
    voiceModeState = 'listening';
    updateVoiceStatus('Listening for T9 string with blank...');
}

function handleTriggerOkay(text) {
    // Extract and apply T9 string filter
    if (voiceModeData.t9String === null) {
        extractT9String(text);
    }
    
    if (voiceModeData.t9String !== null && currentFilteredWords.length > 0) {
        applyT9OneLieFilter();
        showBIdentityOptions();
    }
    
    // Start listening for B-IDENTITY
    voiceModeData.bIdentity = null;
    voiceModeState = 'listening';
    updateVoiceStatus('Listening for B-IDENTITY...');
}

function handleTriggerInteresting(text) {
    // Extract and apply B-IDENTITY filter
    if (voiceModeData.bIdentity === null) {
        extractBIdentity(text);
    }
    
    if (voiceModeData.bIdentity !== null && currentFilteredWords.length > 0) {
        applyBIdentityFilter();
        showFinalResults();
    }
}

// Extract length from voice input
function extractLength(text) {
    ...
    for (const [word, num] of Object.entries(numberWords)) {
        if (lowerText.includes(word)) {
            voiceModeData.length = num;
            updateVoiceStatus(`Length: ${num}`);
            return;
        }
    }
    ...
    if (numbers) {
        voiceModeData.length = parseInt(numbers[0]);
        updateVoiceStatus(`Length: ${voiceModeData.length}`);
    }
}

// Extract T9 string from voice input (e.g., "two, four, blank, eight" = "24B8")
function extractT9String(text) {
    const numberWords = {
        'zero': '0', 'one': '1', 'two': '2', 'three': '3', 'four': '4',
        'five': '5', 'six': '6', 'seven': '7', 'eight': '8', 'nine': '9'
    };
    ...
    for (const part of parts) {
        ...
        if (trimmed === 'blank' || trimmed === 'b') {
            t9String += 'B';
        } else if (numberWords[trimmed]) {
            t9String += numberWords[trimmed];
        } else {
    ...
    if (t9String.length === 4) {
        voiceModeData.t9String = t9String;
        updateVoiceStatus(`T9 String: ${t9String}`);
    }
}

// Extract B-IDENTITY from voice input
function extractBIdentity(text) {
    const numberWords = {
        'zero': '0', 'one': '1', 'two': '2', 'three': '3', 'four': '4',
        'five': '5', 'six': '6', 'seven': '7', 'eight': '8', 'nine': '9'
    };
    ...
    if (digit) {
        voiceModeData.bIdentity = digit[0];
    }
}

// Apply filters
function applyLengthFilter() {
    if (voiceModeData.length === null) return;
    
    // Use existing T9 length filter function
    currentFilteredWords = filterWordsByT9Length(currentFilteredWords, voiceModeData.length);
    updateVoiceResults(`Filtered by length: ${voiceModeData.length} digits<br>Words remaining: ${currentFilteredWords.length}`);
}

function applyT9OneLieFilter() {
    if (!voiceModeData.t9String || voiceModeData.t9String.length !== 4) return;
    
    // Convert string to array (e.g., "23B8" -> ['2', '3', 'BLANK', '8'])
    const digits = voiceModeData.t9String.split('').map(d => d === 'B' ? 'BLANK' : d);
    
    // Store for B-IDENTITY
    t9OneLieSelectedDigits = digits;
    t9OneLieBlankIndex = digits.indexOf('BLANK');
    
    // Apply filter
    currentFilteredWords = filterWordsByT9OneLie(currentFilteredWords, digits);
    
    // Calculate possible digits for blank
    t9OneLiePossibleDigits = calculatePossibleT9DigitsForBlank(currentFilteredWords, digits);
    
    updateVoiceResults(`T9 String: ${voiceModeData.t9String}<br>Words remaining: ${currentFilteredWords.length}`);
}

function applyBIdentityFilter() {
    if (!voiceModeData.bIdentity) return;
    
    currentFilteredWords = filterWordsByT9B(currentFilteredWords, voiceModeData.bIdentity);
    updateVoiceResults(`B-IDENTITY: ${voiceModeData.bIdentity}<br>Words remaining: ${currentFilteredWords.length}`);
}

function showBIdentityOptions() {
    const optionsDiv = document.getElementById('voiceModeBIdentityOptions');
    if (optionsDiv && t9OneLiePossibleDigits.length > 0) {
        optionsDiv.style.display = 'block';
        optionsDiv.innerHTML = `Possible B-IDENTITY options: ${t9OneLiePossibleDigits.join(', ')}`;
    }
}

function showFinalResults() {
    // Show wordlist and T9 string with first 4 digits highlighted
    let results = `Final Results:<br><br>`;
    results += `Words remaining: ${currentFilteredWords.length}<br><br>`;
    
    if (currentFilteredWords.length <= 50) {
        results += `Wordlist:<br>${currentFilteredWords.join(', ')}<br><br>`;
    } else {
        results += `Wordlist (first 50):<br>${currentFilteredWords.slice(0, 50).join(', ')}...<br><br>`;
    }
    
    // Show T9 strings with first 4 digits highlighted
    if (voiceModeData.t9String) {
        results += `T9 String: `;
        const firstFour = voiceModeData.t9String.substring(0, 4);
        const rest = voiceModeData.t9String.substring(4);
        results += `<span class="highlighted">${firstFour}</span>${rest}`;
    }
    
    updateVoiceResults(results);
}

function updateVoiceStatus(text) {
    const statusDiv = document.getElementById('voiceModeStatus');
    if (statusDiv) {
        statusDiv.textContent = text;
    }
}

function updateVoiceResults(text) {
    const resultsDiv = document.getElementById('voiceModeResults');
    if (resultsDiv) {
        resultsDiv.innerHTML = text;
    }
}

// OpenAI Whisper API fallback (if Web Speech API not available)
async function startOpenAIRecognition() {
    if (isRecording) return;
    
    // Check if API key is set
    if (OPENAI_API_KEY === 'YOUR_API_KEY_HERE' || !OPENAI_API_KEY) {
        updateVoiceStatus('OpenAI API key not configured. Please add your API key in script.js');
        return;
    }
    ...
    try {
        ...
        mediaRecorder.onstop = async () => {
            ...
            // Restart recording if still in listening mode
            if (voiceModeActive && voiceModeState === 'listening') {
                setTimeout(() => {
                    startOpenAIRecognition();
                }, 500);
        ...
        mediaRecorder.start();
        isRecording = true;
        updateVoiceStatus('Recording... (OpenAI)');
        
        // Auto-stop after 5 seconds of silence or 30 seconds max
        setTimeout(() => {
...
async function transcribeWithWhisper(audioBlob) {
    try {
        ...
        updateVoiceStatus(`Error: ${error.message}. Tap to retry.`);
